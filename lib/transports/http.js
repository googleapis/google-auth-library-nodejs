...
function formulateRequest (generatedOptions, cb) {
  var body = '';
  var request = getTransport(generatedOptions.isUsingHttps).request(
...
function normalizeRequestOptions (givenOptions) {
  return utils.url.generateUrl(utils.headers.generateHeaders(
    utils.payload.generateRequestPayload({}, givenOptions), givenOptions),
    givenOptions
  );
}
...
function request (givenOptions, cb) {
  return formulateRequest(
    normalizeRequestOptions(givenOptions),
    wrapGivenCallback(cb)
  );
}
