 ...
 */
OAuth2Client.prototype.refreshAccessToken = function(opt_uri, callback) {
  var that = this;
 ...
 */
OAuth2Client.prototype.getAccessToken = function(opt_uri, callback) {
  ...
  if (shouldRefresh && credentials.refresh_token) {
    this.refreshAccessToken(opt_uri, function(err, tokens, response) {
      if (err) {
...
OAuth2Client.prototype.request = function(opts, callback) {
  ...
  var unusedUri = null;
  return this.getAccessToken(unusedUri, cb_with_access_token);
};
