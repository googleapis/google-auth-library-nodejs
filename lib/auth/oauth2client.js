 ...
 */
OAuth2Client.prototype.refreshAccessToken = function(cb_with_metadata) {
  var unusedUri = null;
  this.getRequestMetadata(unusedUri, cb_with_metadata);
};
...
OAuth2Client.prototype.getAccessToken = function(callback) {
  ...
  if (shouldRefresh && credentials.refresh_token) {
    var unusedUri = null;
    this.getRequestMetadata(unusedUri, function(err, tokens, response) {
      if (err) {
...
OAuth2Client.prototype.getRequestMetadata = function(opt_uri, cb_with_metadata) {
  ...
  if (!this.credentials.refresh_token) {
    cb_with_metadata(new Error('No refresh token is set.'), null);
    return;
  }
  ...
  this.refreshToken_(this.credentials.refresh_token, function(err, result, response) {
    if (err) {
      ...
      cb_with_metadata(null, that.credentials, response);
    }
  });
};
...
OAuth2Client.prototype.request = function(opts, callback) {
  ...
  var cb_with_access_token = function(err, access_token, response) {
    if (err) {
  ...
  return this.getAccessToken(cb_with_access_token);
};
