 ...
 * @constructor
 */
function RefreshClient(clientId, clientSecret, refreshToken) {
  RefreshClient.super_.call(this, clientId, clientSecret);
  // Named to avoid collision with the method refreshToken_
  this._refreshToken = refreshToken;
}

util.inherits(RefreshClient, Auth2Client);
 ...
 * @private
 */
RefreshClient.prototype.refreshToken_ = function(ignored_, opt_callback) {
  RefreshClient.super_.prototype.refereshToken_.call(this._refreshToken,
                                                     opt_callback);
};

/**
 * Create a RefreshClient credentials instance using the given input options.
 * @param {object=} json The input object.
 * @param {function=} opt_callback Optional callback.
 */
RefreshClient.prototype.fromJSON = function(json, opt_callback) {
  var that = this;
  if (!json) {
...
/**
 * Create a RefreshClient credentials instance using the given input stream.
 * @param {object=} stream The input stream.
 * @param {function=} opt_callback Optional callback.
 */
RefreshClient.prototype.fromStream = function(stream, opt_callback) {
  var that = this;
  if (!stream) {
