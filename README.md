...
#### Security Considerations
Note that this library does not perform any validation on the token_url, token_info_url, or service_account_impersonation_url values, so it is not recommended to use a JSON credential configuration that was not generated by the gcloud CLI unless you verify that the URL fields point to a googleapis.com domain.
