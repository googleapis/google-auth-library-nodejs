...
async function main() {
  ...
  // Make a simple request to the Google Plus API using our pre-authenticated
  // client. The `request()` method takes an AxiosRequestConfig object.  Visit
...
function getAuthenticatedClient() {
  return new Promise((resolve, reject) => {
    ...
    const authorizeUrl = oAuth2Client.generateAuthUrl({
      ...
      // When using `generateCodeVerifier`, make sure to use
    ...
    const server = http
      .createServer(async (req, res) => {
        if (req.url.indexOf('/oauth2callback') > -1) {
          ...
          // Pass along the generated code verifier that matches our code
          // challenge.
