...
var MockTransporter = /** @class */ (function (_super) {
    __extends(MockTransporter, _super);
    function MockTransporter(simulate_gce, throw_error) {
        ...
        _this.throw_error = throw_error;
        _this.executionCount = 0;
        return _this;
    }
...
}(transporters_1.DefaultTransporter));
...
describe('GoogleAuth', function () {
    describe('.fromJson', function () {
        ...
        describe('.fromAPIKey', function () {
            ...
            describe('Exception behaviour', function () {
                var auth;
                before(function () {
                    auth = new googleauth_1.GoogleAuth();
                });
            ...
            describe('Request/response lifecycle mocking', function () {
                ...
                describe('With preexisting query string parameters', function () {
                    it('should make a request while preserving original parameters', function (done) {
    ...
    describe('.fromStream', function () {
        ...
        it('should read the stream and create a jwt', function (done) {
            ...
            var fileContents = fs.readFileSync('./ts/test/fixtures/private.json', 'utf-8');
            var json = JSON.parse(fileContents);
            // Now open a stream on the same file.
            var stream = fs.createReadStream('./ts/test/fixtures/private.json');
        ...
        it('should read another stream and create a UserRefreshClient', function (done) {
            // Read the contents of the file into a json object.
            var fileContents = fs.readFileSync('./ts/test/fixtures/refresh.json', 'utf-8');
            var json = JSON.parse(fileContents);
            // Now open a stream on the same file.
            var stream = fs.createReadStream('./ts/test/fixtures/refresh.json');
            // And pass it into the fromStream method.
            var auth = new googleauth_1.GoogleAuth();
            auth.fromStream(stream, function (err, result) {
                assert.ifError(err);
    ...
    describe('._getApplicationCredentialsFromFilePath', function () {
        it('should not error on valid symlink', function (done) {
            var auth = new googleauth_1.GoogleAuth();
            auth._getApplicationCredentialsFromFilePath('./ts/test/fixtures/goodlink', function (err) {
        ...
        it('should error on invalid symlink', function (done) {
            var auth = new googleauth_1.GoogleAuth();
            auth._getApplicationCredentialsFromFilePath('./ts/test/fixtures/badlink', function (err) {
                assert.equal(true, err instanceof Error);
                done();
            });
        });
        it('should error on valid link to invalid data', function (done) {
            var auth = new googleauth_1.GoogleAuth();
            auth._getApplicationCredentialsFromFilePath('./ts/test/fixtures/emptylink', function (err) {
                assert.equal(true, err instanceof Error);
                done();
            });
        });
        ...
        it('should error on directory', function (done) {
            ...
            var directory = './ts/test/fixtures';
        ...
        it('should handle errors thrown from createReadStream', function (done) {
            ...
            // Execute.
            auth._getApplicationCredentialsFromFilePath('./ts/test/fixtures/private.json', function (err) {
        ...
        it('should correctly read the file and create a valid JWT', function (done) {
            ...
            var auth = new googleauth_1.GoogleAuth();
            auth._getApplicationCredentialsFromFilePath('./ts/test/fixtures/private.json', function (err, result) {
                assert.equal(null, err);
                assert.equal(json.private_key, result.key);
                assert.equal(json.client_email, result.email);
                assert.equal(null, result.keyFile);
    ...
    describe('._tryGetApplicationCredentialsFromEnvironmentVariable', function () {
        it('should return false when env const is not set', function (done) {
        ...
        it('should return false when env const is empty string', function (done) {
        ...
        it('should handle valid environment variable', function (done) {
            ...
            var auth = new googleauth_1.GoogleAuth();
            insertEnvironmentVariableIntoAuth(auth, 'GOOGLE_APPLICATION_CREDENTIALS', './ts/test/fixtures/private.json');
            // Read the contents of the file into a json object.
            var fileContents = fs.readFileSync('./ts/test/fixtures/private.json', 'utf-8');
            var json = JSON.parse(fileContents);
            // The test ends successfully after 2 steps have completed.
    ...
    describe('.getDefaultProjectId', function () {
        ...
        it('should use GOOGLE_APPLICATION_CREDENTIALS file when it is available', function (done) {
            var projectId = 'my-awesome-project';
            var auth = new googleauth_1.GoogleAuth();
            insertEnvironmentVariableIntoAuth(auth, 'GOOGLE_APPLICATION_CREDENTIALS', path.join(__dirname, '../ts/test/fixtures/private2.json'));
            // Execute.
            auth.getDefaultProjectId(function (err, _projectId) {
                assert.equal(err, null);
                assert.equal(_projectId, projectId);
        ...
        it('should use GCE when well-known file and env const are not set', function (done) {
            var projectId = 'my-awesome-project';
            var auth = new googleauth_1.GoogleAuth();
            blockGoogleApplicationCredentialEnvironmentVariable(auth);
            auth._getSDKDefaultProjectId = function (callback) {
    ...
    describe('.getApplicationDefault', function () {
        it('should return a new credential the first time and a cached credential the second time', function (done) {
            ...
            // on an environment variable json file, but not on anything else.
            var setUpAuthForEnvironmentVariable = function (creds) {
                insertEnvironmentVariableIntoAuth(creds, 'GOOGLE_APPLICATION_CREDENTIALS', './ts/test/fixtures/private.json');
                creds._fileExists = returns(false);
                creds._checkIsGCE = callsBack(null, false);
            };
            // Set up a new GoogleAuth and prepare it for local environment
        ...
        it('should use environment variable when it is set', function (done) {
            ...
            insertWellKnownFilePathIntoAuth(auth, 'foo:gcloud:application_default_credentials.json', './ts/test/fixtures/private2.json');
            // Execute.
            auth.getApplicationDefault(function (err, result) {
                assert.equal(null, err);
                assert.equal(json.private_key, result.key);
        ...
        it('should use well-known file when it is available and env const is not set', function (done) {
            // We expect private2.json to be the file that is used.
            var fileContents = fs.readFileSync('./ts/test/fixtures/private2.json', 'utf-8');
            var json = JSON.parse(fileContents);
            // Set up the creds.
            // * Environment variable is not set.
            // * Well-known file is set up to point to private2.json
        ...
        it('should report GCE error when checking for GCE fails', function (done) {
            // Set up the creds.
            // * Environment variable is not set.
            // * Well-known file is not set.
            // * Running on GCE is set to true.
    ...
    describe('._checkIsGCE', function () {
        ...
        it('Does not execute the second time when not running on GCE', function (done) {
            ...
            it('Returns false on transport error', function (done2) {
                var auth2 = new googleauth_1.GoogleAuth();
                // Mock the transport layer to indicate that we're not running on GCE,
