...
it('should request the identity endpoint when targetAudience is set', async () => {
  ...
  const compute = new Compute({targetAudience});
  await compute.request({url});

  tokenFetchNock.done();
  apiRequestNock.done();

  assert.strictEqual(compute.credentials.id_token, 'abc123');
});
