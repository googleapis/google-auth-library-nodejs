...
describe('impersonated', () => {
  ...
  describe('trust boundaries', () => {
    ...
    it('should send TB header to IAM if it is available', async () => {
      ...
      const scopes = [
        ...
        nock('https://iamcredentials.googleapis.com') // First call to IAM without TB.
          .post(
            '/v1/projects/-/serviceAccounts/target@project.iam.gserviceaccount.com:generateAccessToken',
            (body: ImpersonatedCredentialRequest) => {
              assert.strictEqual(body.lifetime, '30s');
        ...
        setupTrustBoundaryNock(SOURCE_EMAIL),
        setupTrustBoundaryNock(TARGET_PRINCIPAL_EMAIL),
        nock('https://iamcredentials.googleapis.com') // Second call to IAM with TB.
          .post(
            '/v1/projects/-/serviceAccounts/target@project.iam.gserviceaccount.com:generateAccessToken',
            (body: ImpersonatedCredentialRequest) => {
              assert.strictEqual(body.lifetime, '30s');
          ...
          .matchHeader('x-allowed-locations', EXPECTED_TB_DATA.encodedLocations)
          .reply(200, {
            accessToken: MOCK_ACCESS_TOKEN,
            expireTime: tomorrow.toISOString(),
          }),
        setupTrustBoundaryNock(TARGET_PRINCIPAL_EMAIL), // Mock second call to the TB endpoint.
      ];
      await impersonated.getRequestHeaders();

      impersonated.eagerRefreshThresholdMillis = ONE_HOUR_IN_SECS * 1000; // Expire the token
