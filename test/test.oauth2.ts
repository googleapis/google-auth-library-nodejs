...
describe(__filename, () => {
  ...
  it('getToken should set redirect_uri if not provided in options',
     async () => {
       const scope =
           nock(baseUrl)
               .post('/token', undefined, {
  ...
  it('getToken should override redirect_uri if provided in options',
     async () => {
       const scope =
           nock(baseUrl)
               .post('/token', undefined, {
               ...
               .reply(
                   ...
                   {access_token: 'abc', refresh_token: '123', expires_in: 10});
       const res = await client.getToken(
           {code: 'code here', redirect_uri: 'overridden'});
       scope.done();
       assert(res.res);
       if (!res.res) return;
       const params = qs.parse(res.res.config.data);
  ...
  it('should throw if tries to refresh but no refresh token is available',
     async () => {
       client.setCredentials({
         access_token: 'initial-access-token',
         expiry_date: (new Date()).getTime() - 1000
